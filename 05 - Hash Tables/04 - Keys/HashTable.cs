public class HashTable
{
    ...
    {
        // What is the address we should be addressing?
        int address = Hash(key);
        // Iterating through the Linked List, in case this is not null:
        var tempNode = dataMap[address];
        while (tempNode != null) // We won't access the loop if tempNode is null.
        {
            if (tempNode.Key == key)
                return tempNode.Value;
            // Going forward, because we didn't find what we were looking for:
            tempNode = tempNode.Next;
        }
        return null;
    }
    public List<string> Keys()
    {
        ...
        foreach (var location in dataMap) // For each possible addresses:
        {
            var tempNode = location; // Temp node to iterate through the possible Linked Lists
            while (tempNode != null)
            {
                finalList.Add(tempNode.Key!);
                tempNode = tempNode.Next;
            }
        }
        return finalList;
